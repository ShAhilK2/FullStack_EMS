...
const Sidebar = () => {
  ...
  //   Closemobile sidebar on route change
  useEffect(() => {
    setMobileOpen(false);
  }, [pathname]);
  ...
  return (
    <>
      ...
      {mobileOpen && (
        <aside
          className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-900 tet-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
          {SideBarContent()}
        </aside>
      )}
